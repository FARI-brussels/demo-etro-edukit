...
function createGrid() {
  ...
  const material = new THREE.LineBasicMaterial({ color: gridColor, transparent: true, opacity: 0.35 })
  ...
  const gradMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(props.background), transparent: true, opacity: 0.9 })
...
function createGlow() {
  ...
  const glowMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(props.color), transparent: true, opacity: 0.08 })
...
<template>
  <div ref="container" style="width: 100%; height: 460px; border-radius: 10px; overflow: hidden; box-shadow: 0 20px 40px rgba(0,0,0,0.35);"></div>
</template> 